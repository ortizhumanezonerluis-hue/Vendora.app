...
export const dbRepositories = {
  ...
  smartLookupBarcodeOffline(barcode: string) {
    const db = getDatabase()
    const clean = barcode.trim()
    const match = db.prepare('SELECT * FROM catalogo_maestro_offline WHERE barcode = ?').get(clean) as any
    if (match) {
      return {
  ...
  registrarVenta(payload: {
    ...
    const executeCheckout = db.transaction(() => {
      ...
      const config = db.prepare('SELECT prefijo_factura, consecutivo_actual FROM configuracion_negocio WHERE negocio_id = ?').get(negocioId) as any
      ...
      `).run(
        ...
        negocioId,
        consecutivoStr,
        venta.usuario_id || 'usr-admin-principal',
        Number(venta.total) || 0,
        Number(venta.subtotal) || 0,
        Number(venta.iva_total) || 0,
        venta.metodo_pago || 'efectivo',
        Number(venta.monto_recibido) || Number(venta.total) || 0,
      ...
      for (const item of detalles) {
        ...
        insertMov.run(
          ...
          item.producto_id,
          qty,
          `Venta #${consecutivoStr}`,
          venta.usuario_id || 'usr-admin-principal'
        )
      }

      // 4. Actualizar total de ventas en el arqueo de caja abierto si existe
      const arqueoAbierto = db.prepare('SELECT id FROM arqueos_caja WHERE negocio_id = ? AND estado = "abierto" ORDER BY fecha_apertura DESC LIMIT 1').get(negocioId) as any
      if (arqueoAbierto) {
  ...
  cerrarCaja(payload: { id: string; efectivo_declarado: number; observaciones?: string }) {
    const db = getDatabase()
    const arq = db.prepare('SELECT * FROM arqueos_caja WHERE id = ?').get(payload.id) as any
    if (!arq) throw new Error('Arqueo no encontrado')
  ...
  createSesionAuditoria(sesion: any) {
    const db = getDatabase()
    ...
    `).run(
      ...
      sesion.responsable,
      sesion.alcance,
      sesion.filtro_valor || null,
      sesion.ocultar_teorico ? 1 : 0
    )

    return db.prepare('SELECT * FROM sesiones_auditoria WHERE id = ?').get(id)
  },

  saveDetallesAuditoria(sesionId: string, items: any[]) {
    const db = getDatabase()
  ...
  getDetallesAuditoria(sesionId: string) {
    const db = getDatabase()
    return db.prepare(`
  ...
  getConfiguracion(negocioId: string = 'negocio-local-principal') {
    const db = getDatabase()
    return db.prepare('SELECT * FROM configuracion_negocio WHERE negocio_id = ?').get(negocioId)
  },

  saveConfiguracion(cfg: any) {
    const db = getDatabase()
